...
This page covers all Docker-based deployment options for **ForMD**.
...
docker run -p 8080:80 formd:local
```
...
To serve the app at a **sub-path** (e.g., `/formd/`), adjust the Nginx `location` directive in the `Dockerfile` and rebuild the image.
...
To run ForMD behind a reverse proxy (Nginx, Caddy, Traefik, Apache), proxy requests to the container's port 80.
