...
This page covers the **Neutralinojs desktop application** port of ForMD — a lightweight, cross-platform native app built from the same source code as the web version.
...
- [Building the Desktop App](#building-the-Formd-Desktop)
...
The `prepare.js` script handles copying files from the project root into `formd-desktop/resources/` before each build so there is a single source of truth.
...
After building, output files are placed in `formd-desktop/dist/`:

```
dist/
├── formd-win_x64.exe       # Windows embedded binary
├── formd-linux_x64         # Linux x64 embedded binary
├── formd-linux_arm64       # Linux ARM64 embedded binary
├── formd-mac_universal     # macOS universal binary
...
| File | Platform |
|------|----------|
| `formd-win_x64.exe` | Windows x64 |
| `formd-linux_x64` | Linux x64 |
| `formd-linux_arm64` | Linux ARM64 |
| `formd-mac_universal` | macOS (Apple Silicon + Intel) |
